(function () {
  ...
  function scoreReport(report) {
    ...
    if (report.cdpStackCheckProbe && report.cdpStackCheckProbe.detected) {
      addFinding(findings, "Inspector/CDP", "CDP Stack Check 命中", "Error.prepareStackTrace 在 " + report.cdpStackCheckProbe.detectedContexts.join(", ") + " 上下文被触发，说明 console Error 栈被 Inspector/CDP 消费者物化。", 28, "danger");
    }

    if (report.cdpObjectInspectionProbe && report.cdpObjectInspectionProbe.hits > 0) {
      addFinding(findings, "Inspector/CDP", "Error 对象内省探针命中", "自动化端或调试端读取了页面暴露的新 Error 对象，导致 stack 被物化。", 18, "warn");
    }
